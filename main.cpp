...
void SetWorkingDirectoryToBundleResources() {
    ...
    if (_NSGetExecutablePath(path, &size) == 0) {
        ...
        if (realPath != NULL) {
            ...
            chdir(dir); 
            free(realPath);
            TraceLog(LOG_INFO, "SYSTEM: Working directory set to bundle executable path.");
        }
    }
}
#endif
